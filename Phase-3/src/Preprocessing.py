...
def pre_process_task2():
    ...
    print("started D_out at", time.asctime( time.localtime(time.time()) ))
    D_out = np.eye(A.shape[0]) * np.sum(A, axis=1, keepdims=True)
    D_out = np.sqrt(pinv(D_out))
    D_out = scipy.sparse.csr_matrix(D_out)
    print("ended D_out at", time.asctime( time.localtime(time.time()) ))

    print("started D_in at", time.asctime( time.localtime(time.time()) ))
    D_in = np.eye(A.shape[0]) * np.sum(A, axis=0, keepdims=True)
    D_in = np.sqrt(pinv(D_in))
    D_in = scipy.sparse.csr_matrix(D_in)
    print("ended D_in at", time.asctime( time.localtime(time.time()) ))

    print("started calculating B at", time.asctime( time.localtime(time.time()) ))
    B = csr_matrix.dot(csr_matrix.dot(csr_matrix.dot(csr_matrix.dot(D_out,A),D_in),A.T),D_out)
    print("started calculating B at", time.asctime( time.localtime(time.time()) ))

    print("started calculating C at", time.asctime( time.localtime(time.time()) ))
